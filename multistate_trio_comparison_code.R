...
grain_3 <- grains %>% 
  filter(site %in% c("CORNELL", "KENTUCKY LEXINGTON", "WISCONSIN" ))
grain_3$site <- factor(grain_3$site, levels=c("CORNELL", "KENTUCKY LEXINGTON", "WISCONSIN"))
...
straw_3 <- straws %>% 
  filter(site %in% c( "CORNELL", "KENTUCKY LEXINGTON", "WISCONSIN"))
straw_3$site <- factor(straw_3$site, levels=c("CORNELL", "KENTUCKY LEXINGTON", "WISCONSIN"))

ggplot(straw_3,aes(x=variety,y=std_straw))+
  geom_bar(stat="identity") +
  facet_wrap(vars(site)) +
  labs(x = "Variety", y = "Straw Yield Z") +
  theme_bw(base_size = 12) +
